...
export class GlobalDecoderRegistry {
  ...
  static register<T, SDK, Amino>(key: string, decoder: TelescopeGeneratedCodec<T, SDK, Amino>) {
  ...
  static getDecoderByInstance<T, SDK, Amino>(obj: unknown): TelescopeGeneratedCodec<T, SDK, Amino> | null {
    ...
    if (protoType.$typeUrl) {
      return GlobalDecoderRegistry.getDecoder<T, SDK, Amino>(protoType.$typeUrl);
    }
  ...
  static unwrapAny<T, SDK, Amino>(input: BinaryReader | Uint8Array | Any) {
    ...
    const decoder = GlobalDecoderRegistry.getDecoder<T, SDK, Amino>(data.typeUrl);
  ...
  static fromAminoMsg<T = unknown, Amino = unknown>(object: AnyAmino): T {
    const decoder = GlobalDecoderRegistry.getDecoderByAminoType<T, unknown, Amino>(object.type);
...
function getDecoderByInstance<T = unknown, SDK = unknown, Amino = unknown>(
  ...
  const decoder = GlobalDecoderRegistry.getDecoderByInstance<T, SDK, Amino>(obj);
